...
export const VideoPlayer: React.FC<VideoPlayerProps> = ({
  ...
  const handleErrorCode = (event: YouTubeEvent<number>) => {
  ...
  return (
    ...
    >
      ...
      {embedBlocked && (
        <div className="absolute inset-0 z-30 bg-theme-elevated flex items-center justify-center">
          <div className="text-center px-8 py-6 max-w-md">
            <div className="w-16 h-16 mx-auto mb-4 rounded-2xl bg-status-warning/10 flex items-center justify-center">
              <svg className="w-8 h-8 text-status-warning" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
            </div>
            <h3 className="text-theme-primary font-semibold text-lg mb-2">Embedding not available</h3>
            <p className="text-theme-secondary text-sm mb-4">This video can't be played here. The creator has disabled embedding.</p>
            <a
              href={`https://www.youtube.com/watch?v=${video?.id}`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2 bg-theme-accent text-on-accent rounded-lg text-sm font-medium hover:brightness-110 transition-all"
            >
              Watch on YouTube
