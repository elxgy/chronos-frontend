...
export const Modal: React.FC<ModalProps> = ({
  ...
  const modalContent = (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
      <div
        className="absolute inset-0 bg-theme-root/80 backdrop-blur-sm cursor-default"
        onClick={onClose}
        aria-hidden="true"
      />
      <div
        className={cn(
          'relative w-full bg-theme-elevated rounded-xl border-2 border-theme shadow-2xl animate-fade-in max-h-[90vh] overflow-auto',
          sizes[size]
        )}
      >
        {(title || showCloseButton) && (
          <div className="flex items-start justify-between p-5 border-b-2 border-theme">
            <div>
              {title && (
